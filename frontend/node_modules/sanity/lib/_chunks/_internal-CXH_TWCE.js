...
const request$1 = getIt([keepAlive(), promise()]);
...
const debug = debug$3("sanity:backup");
...
const downloadBackupCommand = {
  ...
  action: async (args, context) => {
    ...
    progressSpinner.set({
      step: "Backup download complete [".concat(prettyMs(Date.now() - start), "]")
    });
...
async function getBuildAction() {
  const mod = await import('./buildAction-D37qdTUB.js');
  return mod.default;
}
const checkCommand = {
  name: "check",
...
const addCorsOriginCommand = {
  ...
  action: async (args, context) => {
    ...
    const isFile = fs.existsSync(path$1.join(process.cwd(), origin));
    if (isFile) {
  ...
  action: async (args, context) => {
    ...
    if (!destinationPath) {
      destinationPath = await prompt.single({
        type: "input",
        message: "Output path:",
        default: path$1.join(workDir, "".concat(dataset, ".tar.gz")),
        filter: absolutify
      });
    }
...
async function getOutputPath(destination, dataset, prompt, flags) {
  ...
  const dstPath = path$1.isAbsolute(destination) ? destination : path$1.resolve(process.cwd(), destination);
  let dstStats = await fs$1.stat(dstPath).catch(noop);
  const looksLikeFile = dstStats ? dstStats.isFile() : path$1.basename(dstPath).indexOf(".") !== -1;
  if (!dstStats) {
    const createPath = looksLikeFile ? path$1.dirname(dstPath) : dstPath;
    await fs$1.mkdir(createPath, {
      recursive: true
    });
  }
  const finalPath = looksLikeFile ? dstPath : path$1.join(dstPath, "".concat(dataset, ".tar.gz"));
  dstStats = await fs$1.stat(finalPath).catch(noop);
...
const importDatasetCommand = {
  ...
  action: async (args, context) => {
    ...
    if (isUrl) {
      ...
      const sourceFile = path$1.resolve(process.cwd(), file);
      const fileStats = await fs$1.stat(sourceFile).catch(() => null);
      if (!fileStats) {
      ...
      if (sourceIsFolder) {
        ...
        assetsBase = path$1.dirname(sourceFile);
...
const deployCommand = {
  ...
  action: async (args, context) => {
    const mod = await import('./deployAction-YwQ2Jwcx.js');
...
const undeployCommand = {
  ...
  action: async (args, context) => {
    const mod = await import('./undeployAction-5HkuC4OH.js');
    return mod.default(args, context);
  },
  helpText: helpText$j
};
...
async function getDevAction() {
  const mod = await import('./devAction-F8mMTJzH.js');
  return mod.default;
}
...
const createDocumentsCommand = {
  ...
  action: async (args, context) => {
    ...
    if (file) {
      const contentPath = path$1.resolve(process.cwd(), file);
      const content = json5.parse(await fs$1.readFile(contentPath, "utf8"));
    ...
    const tmpFile = path$1.join(os.tmpdir(), "sanity-cli", "".concat(docId, ".").concat(ext));
    const stringify = useJson5 ? json5.stringify : JSON.stringify;
    const defaultValue = id && (await client.getDocument(id)) || {
      _id: docId,
      _type: "specify-me"
    };
    await fs$1.mkdir(path$1.join(os.tmpdir(), "sanity-cli"), {
      recursive: true
    });
    ...
    if (watch) {
      ...
      await fs$1.unlink(tmpFile).catch(noop$1);
    }
...
const description$2 = "Downloads and validates all document specified in a workspace";
...
const validateDocumentsCommand$1 = {
  ...
  helpText: helpText$d,
  action: async (args, context) => {
    const mod = await import('./validateAction-Do76vhqC.js');
    return mod.default(args, context);
  }
};
...
const execCommand = {
  ...
  helpText: helpText$c,
  action: async (args, context) => {
    const mod = await import('./execScript-BGLoBwD7.js');
    return mod.default(args, context);
  }
};
...
const deleteGraphQLAPICommand = {
  ...
  action: async (args, context) => {
    const mod = await import('./deleteApiAction-z-wsioF2.js');
    return mod.default(args, context);
  },
  helpText: helpText$b
};
...
const deployGraphQLAPICommand = {
  ...
  action: async (args, context) => {
    const mod = await import('./deployApiAction-BQBvK00X.js');
    return mod.default(args, context);
  },
  helpText: helpText$a
};
...
const listGraphQLAPIsCommand = {
  ...
  action: async (args, context) => {
    const mod = await import('./listApisAction-DVp9fGrB.js');
    return mod.default(args, context);
  },
  helpText: helpText$9
};
...
function removeMigrationScriptExtension(fileName) {
  return MIGRATION_SCRIPT_EXTENSIONS.reduce((name, ext) => name.endsWith(".".concat(ext)) ? path.basename(name, ".".concat(ext)) : name, fileName);
}
var migrationGroup = {
  name: "migration",
  signature: "[COMMAND]",
...
const runMigrationCommand = {
  ...
  action: async (args, context) => {
    ...
    if (resolvedScripts.length > 1) {
      throw new Error('Found multiple migrations for "'.concat(id, '" in ').concat(chalk.cyan(migrationsDirectoryPath), ": \n - ").concat(candidates.map(candidate => path.relative(migrationsDirectoryPath, candidate.absolutePath)).join("\n - ")));
    }
    const script = resolvedScripts[0];
    if (!script) {
      throw new Error('No migration found for "'.concat(id, '" in ').concat(chalk.cyan(chalk.cyan(migrationsDirectoryPath)), ". Make sure that the migration file exists and exports a valid migration as its default export.\n\n Tried the following files:\n - ").concat(candidates.map(candidate => path.relative(migrationsDirectoryPath, candidate.absolutePath)).join("\n - ")));
    }
...
async function getPreviewAction$1() {
  const mod = await import('./previewAction-CzsIbY6e.js');
  return mod.default;
}
const description$1 = "Extracts a JSON representation of a Sanity schema within a Studio context.";
...
const extractSchemaCommand = {
  ...
  hideFromHelp: true,
  action: async (args, context) => {
    const mod = await import('./extractAction-CwHvSxr-.js');
    return mod.default(args, context);
  }
};
var schemaGroup = {
...
const validateDocumentsCommand = {
  ...
  helpText: helpText$3,
  action: async (args, context) => {
    const mod = await import('./validateAction-x8IBi_A2.js');
    return mod.default(args, context);
  }
};
...
const listUsersCommand = {
  ...
  action: async (args, context) => {
    ...
    const ordered = sortBy(members.map(_ref14 => {
      let {
        id,
        name,
        role,
