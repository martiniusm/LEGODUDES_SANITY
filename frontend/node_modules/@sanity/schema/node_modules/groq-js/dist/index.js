...
function handleAccessAttributeNode(node, scope) {
  ...
  return node.base && (attributeBase = walk({ node: node.base, scope })), $trace("accessAttribute.base %s %O", node.name, attributeBase), mapObject(attributeBase, scope, (base) => {
    const attribute = base.attributes[node.name];
    return attribute !== void 0 ? ($debug(`accessAttribute.attribute found ${node.name} %O`, attribute), attribute.optional ? nullUnion(attribute.value) : attribute.value) : ($warn(`attribute "${node.name}" not found in object`), { type: "null" });
  });
}
