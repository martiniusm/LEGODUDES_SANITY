...
function moveRangeByOperation(range, operation) {
  const anchor = slate.Point.transform(range.anchor, operation);
  const focus = slate.Point.transform(range.focus, operation);
  if (anchor === null || focus === null) {
    return null;
  }
  if (slate.Point.equals(anchor, range.anchor) && slate.Point.equals(focus, range.focus)) {
    return range;
  }
  return {
    anchor,
  ...
  return function withEditableAPI(editor) {
    portableTextEditor.setEditable({
      ...
      isSelectionsOverlapping: (selectionA, selectionB) => {
        ...
        const isValidRanges = slate.Range.isRange(rangeA) && slate.Range.isRange(rangeB);
        const isOverlapping = isValidRanges && slate.Range.includes(rangeA, rangeB);
...
function unsetPatch(editor, patch, previousPatch) {
  ...
  if (editor.isTextBlock(block) && patch.path[1] === "children" && patch.path.length === 3) {
    ...
    debugState(editor, "before");
    if (debugVerbose$3) {
      debug$g("Removing child at path ".concat(JSON.stringify(childPath)));
    }
    slate.Transforms.removeNodes(editor, {
      at: childPath
    });
...
PortableTextEditor.undo = editor => {
  var _a;
  debug$2("Host undoing");
  (_a = editor.editable) == null ? void 0 : _a.undo();
};
PortableTextEditor.redo = editor => {
  var _a;
  debug$2("Host redoing");
  (_a = editor.editable) == null ? void 0 : _a.redo();
};
PortableTextEditor.isSelectionsOverlapping = (editor, selectionA, selectionB) => {
  var _a;
...
const PortableTextEditable = react.forwardRef(function PortableTextEditable2(props, forwardedRef) {
  ...
  const [rangeDecorationState, setRangeDecorationsState] = react.useState(EMPTY_DECORATORS);
  const {
    change$,
    schemaTypes
  } = portableTextEditor;
  ...
  const syncRangeDecorations = react.useCallback(operation => {
  ...
  const decorate = react.useCallback(_ref19 => {
    let [, path] = _ref19;
