...
function toPortableTextRange(value, range, types) {
  ...
  const backward = Boolean(Range.isRange(range) ? Range.isBackward(range) : void 0);
  return anchor && focus ? {
    anchor,
    focus,
    backward
...
const PortableTextEditable = forwardRef(function PortableTextEditable2(props, forwardedRef) {
  ...
  const [rangeDecorationState, setRangeDecorationsState] = useState(EMPTY_DECORATORS);
  const {
    change$,
    schemaTypes
  } = portableTextEditor;
  ...
  const syncRangeDecorations = useCallback(operation => {
  ...
  const decorate = useCallback(_ref19 => {
    let [, path] = _ref19;
